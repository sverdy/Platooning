...
public:
  ...
  uint8_t ApplicationFunctionSet_Tracking(uint8_t vitesse_courante);           //循迹
  void ApplicationFunctionSet_SensorDataUpdate(void);   //传感器数据更新
  void ApplicationFunctionSet_SerialPortDataAnalysis(void);
  uint8_t ApplicationFunctionSet::ApplicationFunctionSet_Platooning(uint8_t vitesse_courante); // fonction de suivi du vehicule précédent
